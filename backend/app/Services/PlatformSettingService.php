...
final class PlatformSettingService
{
    ...
    /**
     * Public "Contact us" links shown in the mobile app (seeker + employer).
     *
     * @return array{
    ...
    {
        $phone = $this->string('contact.support_phone', '9036980547');
        $digits = preg_replace('/\D+/', '', $phone) ?: '9036980547';
        $waDefault = 'https://wa.me/91'.$digits;

        return [
            'support_phone' => $phone,
            'youtube_url' => $this->string('contact.youtube_url', 'https://www.youtube.com/@joballocate'),
            'facebook_url' => $this->string('contact.facebook_url', 'https://www.facebook.com/joballocate'),
            'whatsapp_url' => $this->string('contact.whatsapp_url', $waDefault),
            'instagram_url' => $this->string('contact.instagram_url', 'https://www.instagram.com/joballocate'),
            'linkedin_url' => $this->string('contact.linkedin_url', 'https://www.linkedin.com/company/joballocate'),
            'website_url' => $this->string('contact.website_url', 'https://joballocate.tech'),
            'about_text' => $this->string(
                'contact.about_text',
                'Empowering careers through smart matching and powerful resume building tools.'
            ),
            'app_download_url' => $this->string(
                'refer_earn.app_download_url',
     ...
     */
    public function updateContactSettings(array $data, ?int $updatedBy = null): array
    {
        $map = [
        ...
        foreach ($map as $field => $key) {
            ...
            PlatformSetting::query()->updateOrCreate(
                ['key' => $key],
                [
                    'value' => ['text' => trim((string) ($data[$field] ?? ''))],
                    'updated_by' => $updatedBy,
                ]
            );
        }
