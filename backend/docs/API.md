...
- `POST /job-seeker/packages/select` — body: `{ "package_key": "basic" | "standard" | "premium" }` — activates package **without payment** (dev/admin helper; production apps use Cashfree)
- `POST /job-seeker/payments/create-order` — body: `{ "package_key" }` — creates Cashfree SDK order; returns `merchant_order_id`, `order_id`, `payment_session_id`, `environment` (`sandbox`|`production`), `amount` (paise)
- `POST /job-seeker/payments/confirm-status` — body: `{ "merchant_order_id" }` — calls Cashfree Get Order API; activates package when `PAID`
- `POST /job-seeker/resume/pdf-create-order` — body: `{ "resume_template_id", "resume_template_title" }` — Cashfree order for PDF export; confirm via `payments/confirm-status`
...
### Company subscription (Cashfree)

- `POST /company/subscription/purchase` — creates Cashfree SDK order (same response shape as seeker create-order)
- `POST /company/subscription/confirm-status` — body: `{ "merchant_order_id" }`
- `GET /company/subscription/history`

### Cashfree webhook (public)

- `POST /payments/webhook` — Cashfree callbacks (`PAYMENT_SUCCESS_WEBHOOK` / `PAYMENT_FAILED_WEBHOOK`)
- Auth: HMAC signature via headers `x-webhook-signature` + `x-webhook-timestamp` (uses `CASHFREE_SECRET_KEY`)
