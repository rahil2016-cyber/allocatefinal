...
use App\Http\Controllers\ReferralInviteLandingController;
...
/*
| Referral invite: opens app with code if installed, else Play Store (+ install referrer).
*/
Route::get('/invite/{code}', [ReferralInviteLandingController::class, 'show'])
...
/*
| Android App Links verification (Play Console Digital Asset Links).
| Must be public, HTTPS, and Content-Type: application/json.
*/
...
|  /              → public landing (app info + download)
|  /buildwipe     → secret admin login (SPA route; not linked publicly)
|  /dashboard…    → admin app (auth-gated in SPA)
| Admin deep links must be registered after /media/* so those paths are not swallowed.
| /assets/* is served as static files from public/assets.
*/
Route::get('/{any}', $serveAdminSpa)
    ->where('any', '^(?!terms-and-conditions$|privacy-policy$|refund-policy$|privacy$|share\/job\/\d+|invite\/[A-Za-z0-9_-]+$|\.well-known\/).*');
