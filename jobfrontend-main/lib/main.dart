...
import 'screens/common/legal_webview_screen.dart';
...
class _JobAllocateAppState extends State<JobAllocateApp> {
  ...
  Future<void> _onPushNotification(RemoteMessage message) async {
    ...
    // Foreground receive uses empty/"received" marker — only navigate on taps.
    if (data['__event'] == 'received') return;

    final nav = rootNavigatorKey.currentState;
    if (nav == null) return;

    final type = data['type']?.toString() ?? '';
    final jobIdStr = data['job_id']?.toString() ??
        data['reference_id']?.toString() ??
        data['referenceId']?.toString();

    if ((type == 'job' || type == 'new_job') &&
        jobIdStr != null &&
        jobIdStr.isNotEmpty) {
      try {
