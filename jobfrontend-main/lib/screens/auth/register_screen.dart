...
class _RegisterScreenState extends State<RegisterScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadPendingReferralCode() async {
    final code = await ReferralInviteService.instance.peekPendingCode();
    if (!mounted || code == null || code.isEmpty) return;
    if (_referralCodeController.text.trim().isNotEmpty) return;
    setState(() {
      _referralCodeController.text = code;
      _referralValid = null;
      _referralMessage = 'Code from your invite link — leave it or change if needed.';
    });
    // Soft-validate so registration can proceed without an extra tap.
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Enter code if you have one (auto-filled from invite)',
  ...
  Future<void> _registerToBackend(String idToken) async {
    ...
    try {
      ...
      if (referralCode != null && referralCode.isNotEmpty) {
        await ReferralInviteService.instance.clearPendingCode();
      }
      setState(() {
        _isLoading = false;
        _step = 2;
