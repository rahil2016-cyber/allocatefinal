...
class _PostJobScreenState extends State<PostJobScreen> {
  ...
  Future<void> _callCustomerSupport() async {
    final settings = await ContactSettingsService.instance.load();
    final phone = (settings['support_phone'] ?? '9036980547').trim();
    final uri = Uri.parse(ContactSettingsService.instance.dialUri(phone));
    try {
  ...
  void initState() {
    super.initState();
    ContactSettingsService.instance.load();
    _fetchProfileData();
    final j = widget.existingJob;
    if (j == null) {
      _initTimings();
  ...
  Widget _buildJobDetailsStep(TextTheme textTheme) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 24),
        _buildSectionHeader('Salary Range (₹ / Month)'),
        const SizedBox(height: 16),
        Row(
          children: [
            Expanded(
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Expanded(child: _buildLabel('Work Timings *')),
            TextButton(
              onPressed: () {
                setState(() {
        ...
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Expanded(child: _buildLabel('Interview Would Be Done Between')),
            TextButton(
              onPressed: () {
                setState(() {
  ...
  Future<bool> _ensureCanCreateJob() async {
    ...
    try {
      ...
      if (proceedData != true || !mounted) return false;

      await subApi.purchase(couponCode: null);
      return true;
    } catch (e) {
      if (!mounted) return false;
      ScaffoldMessenger.of(context).showSnackBar(
