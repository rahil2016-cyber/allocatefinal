...
Future<void> _navigateResumeHtmlPreview(
  ...
  final isLocked = !ResumePlanService.instance.isTemplateUnlocked(key);
  
  if (isLocked) {
    // For locked templates, load the demo variant so the backend doesn't throw a 403.
    // The frontend lock overlay will be shown automatically.
    await Navigator.of(context).push<void>(
      MaterialPageRoute<void>(
...
class _JobFeedPageState extends State<_JobFeedPage>
  ...
  Future<void> _hydrateBannersFromCache() async {
    final cached = await BannerApiService.instance.loadCachedBanners();
    if (!mounted || cached.isEmpty) return;
    setState(() => _banners = cached);
  }
  ...
  Future<List<banner_model.PromoBanner>> _fetchBanners({
    bool forceRefresh = false,
  }) async {
    try {
      return await BannerApiService.instance
          .getActiveBanners(forceRefresh: forceRefresh);
    } catch (e) {
  ...
  bool _bannersChanged(
    List<banner_model.PromoBanner> a,
    List<banner_model.PromoBanner> b,
  ) {
    if (identical(a, b)) return false;
    if (a.length != b.length) return true;
    for (var i = 0; i < a.length; i++) {
      if (a[i].id != b[i].id || a[i].imageUrl != b[i].imageUrl) return true;
    }
    return false;
  }
      ...
      if (mounted) {
        setState(() {
          ...
          _referralCode = data['my_referral_code']?.toString() ??
              data['code']?.toString() ??
              '';
          _referralShareMessage = data['share_message']?.toString() ?? '';
  ...
  Future<void> _loadJobs({bool forceBannerRefresh = false}) async {
    setState(() {
      _isLoading = true;
    ...
    try {
      final banners = await _fetchBanners(forceRefresh: forceBannerRefresh);
      if (mounted && _bannersChanged(_banners, banners)) {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () async {
        await Future.wait([
          _loadJobs(forceBannerRefresh: true),
          _loadSavedJobIds(),
          _refreshAppliedIds(),
        ]);
        if (mounted) {
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      final msg = _referralShareMessage.trim().isNotEmpty
                          ? _referralShareMessage
                          : 'Join JobAllocate!\nUse my referral code: $_referralCode\n'
                              'Invite: https://joballocate.tech/invite/$_referralCode\n'
                              'Download: https://play.google.com/store/apps/details?id=com.joballocate.in';
                      Share.share(msg);
                    },
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final htmlSlots = plan.getAvailableTemplates();
        
        // Always show all templates on the home page so users can discover them.
        final List<Map<String, String>> templatesToShow = kSeekerResumeHtmlTemplates;
        ...
        return Column(
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    !plan.hasActivePlan 
                        ? 'Locked' 
                        : (plan.selectedTemplateIds.isEmpty && !plan.isSelectionComplete 
                            ? 'Select templates' 
                            : '${plan.selectedTemplateIds.length} unlocked'),
                    style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.bold,
                      color: AppColors.primary,
            ...
            const SizedBox(height: 24),
            if (plan.hasActivePlan && htmlSlots.isEmpty)
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 24),
                child: Container(
                  width: double.infinity,
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Choose your ${plan.allowedTemplateCount} resumes',
                        style: tt.titleMedium?.copyWith(fontWeight: FontWeight.w800),
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Your plan is active. Select which templates to unlock before editing or downloading.',
                        style: tt.bodyMedium?.copyWith(
                          color: AppColors.textSecondary,
                          height: 1.35,
                        ),
                      ...
                      FilledButton(
                        ...
                        child: const Text('Select templates'),
                      ),
                    ],
                  ),
                ),
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final slot = templatesToShow[index];
                    final key = slot['key'] ?? 't1_teal_sidebar';
                    final label = slot['label'] ?? 'Template';
                    final variant =
                        index % ResumeDemoProfilesCache.instance.variantCount;
                    final isLocked = !plan.isTemplateUnlocked(key);
                    ...
                    return ResumeDashboardTemplateCard(
                      ...
                      onEdit: () => _navigateSeekerResumeStudio(
                        context,
                        templateIdOverride:
                            seekerStudioTemplateIdForHtmlKey(key),
                      ),
                    );
                  },
                ),
