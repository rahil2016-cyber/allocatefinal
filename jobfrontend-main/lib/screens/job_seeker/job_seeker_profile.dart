...
class _JobSeekerProfileScreenState extends State<JobSeekerProfileScreen>
  ...
  Future<void> _load({bool allowAutoRetry = true}) async {
    ...
    try {
      ...
      if (!mounted) return;
      // Transient server lock / blip — retry once silently so users rarely see Retry.
      if (allowAutoRetry) {
  ...
  Future<void> _pickAndUploadResume() async {
    ...
    try {
      ...
      await JobSeekerApiService.instance.uploadResumePdf(
        File(path),
      );
  ...
  Future<void> _deleteResume() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete your PDF resume?'),
        actions: [
          TextButton(
    ...
    setState(() => _uploadingResume = true);
    try {
      await JobSeekerApiService.instance.updateSeekerProfile({'resume_url': null});
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
                    ...
                    _buildSectionCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            smin != null || smax != null
                                ? 'Expected salary: ₹${smin ?? '—'} – ₹${smax ?? '—'} / month'
  ...
  Widget _buildPersonalDetailsCard(Map<String, dynamic>? p, TextTheme textTheme) {
    String dob = p?['dob']?.toString().trim() ?? '';
    if (dob.contains('T')) {
