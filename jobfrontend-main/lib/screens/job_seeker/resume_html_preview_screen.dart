...
const List<Map<String, String>> kSeekerResumeHtmlTemplates = [
  ...
  {'key': 't13_academic_clean', 'label': 'Academic · clean'},
];
...
class _ResumeHtmlPreviewScreenState extends State<ResumeHtmlPreviewScreen> {
  ...
  bool get _isCurrentLocked =>
      !ResumePlanService.instance.isTemplateUnlocked(_templateKey);

  String get _cacheKey {
    if (_isCurrentLocked) {
      return '${_templateKey}_demo_0|${widget.contentEnvelope?.hashCode ?? 0}';
    }
    return '${_templateKey}_${widget.resumeDraftId}_${_useDemo ? widget.demoVariant : 'user'}|${widget.contentEnvelope?.hashCode ?? 0}';
  }
    ...
    try {
      ...
      final html = cached ??
          (await JobSeekerApiService.instance.resumePreviewHtml(
            ...
            contentEnvelope: _isCurrentLocked ? null : widget.contentEnvelope,
            resumeDraftId: _isCurrentLocked ? null : widget.resumeDraftId,
            demoVariant: _isCurrentLocked ? 0 : (_useDemo ? widget.demoVariant : null),
          ))['html']
              ?.toString() ??
          '';
  ...
  Future<void> _onPurchaseAndDownload() async {
    if (!AppSession.isLoggedIn) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Sign in to download your resume PDF.')),
      );
      return;
    }
    ...
    if (!plan.isTemplateUnlocked(_templateKey)) {
    ...
    // Plan purchase already includes PDF downloads for selected templates.
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Opening PDF… Included with your plan.'),
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        ListenableBuilder(
                          ...
                          builder: (context, _) {
                            final unlocked = ResumePlanService.instance.isTemplateUnlocked(_templateKey);
                              ...
                              child: ColoredBox(
                                ...
                                child: Center(
                                  child: Column(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      const Icon(Icons.lock_rounded, size: 64, color: Colors.white),
                                      const SizedBox(height: 16),
                                      const Text(
                                        'Template Locked',
                                      ...
                                      Padding(
                                        ...
                                        child: Text(
                                          'Unlock this template with a plan to remove this lock and download.',
                                          textAlign: TextAlign.center,
                  ...
                  child: SafeArea(
                    ...
                    child: Padding(
                      ...
                      child: ListenableBuilder(
                        listenable: ResumePlanService.instance,
                        builder: (context, _) {
                          final unlocked = ResumePlanService.instance
                              .isTemplateUnlocked(_templateKey);
                          return Row(
                            children: [
                              Expanded(
                                child: FilledButton.icon(
                                  ...
                                  label: Text(
                                    _purchasing
                                        ? 'Processing…'
                                        : unlocked
                                            ? 'Download PDF · Free with plan'
                                            : 'Unlock with a plan to download',
                                  ),
