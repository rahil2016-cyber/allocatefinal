...
class _ResumeSelectionScreenState extends State<ResumeSelectionScreen> {
  ...
  Future<void> _loadSelectionData() async {
    ...
    try {
      ...
      var allowed = res['allowed_count'] is int
          ? res['allowed_count'] as int
          : int.tryParse(res['allowed_count']?.toString() ?? '') ?? 0;
      var key = res['active_package_key']?.toString();
      if (key == null || key.isEmpty) {
        key = widget.initialPackageKey;
      }
      // Just after purchase, selection API may lag — use package key from checkout.
      if (allowed < 1 && key != null) {
        allowed = ResumePlanService.templateLimitForPackage(key);
      }
      ...
      if (existing is List) {
        for (final item in existing) {
          ...
          if (id.isNotEmpty) preselected.add(id);
        }
      }

      // Soft hint only: first N pre-checked; user can change before saving.
      if (preselected.isEmpty && allowed > 0) {
        for (var i = 0; i < allowed && i < kSeekerResumeHtmlTemplates.length; i++) {
          preselected.add(kSeekerResumeHtmlTemplates[i]['key']!);
        }
      } else if (preselected.length > allowed && allowed > 0) {
        final trimmed = preselected.take(allowed).toSet();
        preselected
          ..clear()
          ..addAll(trimmed);
      }
      ...
      if (mounted) {
        final key = widget.initialPackageKey ??
            ResumePlanService.instance.activePackageKey;
        final allowed = key != null
            ? ResumePlanService.templateLimitForPackage(key)
            : ResumePlanService.instance.allowedTemplateCount;
        setState(() {
          _allowedCount = allowed;
          _packageKey = key ?? 'none';
          _selectedKeys.clear();
          for (var i = 0; i < allowed && i < kSeekerResumeHtmlTemplates.length; i++) {
