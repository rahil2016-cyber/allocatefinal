...
class FcmFlutterService {
        ...
        onDidReceiveNotificationResponse: (NotificationResponse response) {
          ...
          if (response.payload != null && response.payload!.isNotEmpty) {
            ...
            NotificationEvents.notify(
              const RemoteMessage(data: {'__event': 'tap'}),
            );
          }
        },
      );
      ...
      messaging.getInitialMessage().then((initialMessage) {
        if (initialMessage != null) {
          ...
          Future.delayed(const Duration(milliseconds: 1000), () {
            final tapped = Map<String, dynamic>.from(initialMessage.data);
            tapped['__event'] = 'tap';
            NotificationEvents.notify(RemoteMessage(data: tapped));
          });
        }
      });
  ...
  void _onForegroundMessage(RemoteMessage message) {
    ...
    // Badge refresh only — do not navigate on receive.
    final received = Map<String, dynamic>.from(message.data);
    received['__event'] = 'received';
    NotificationEvents.notify(RemoteMessage(data: received));
  }

  void _onNotificationTap(RemoteMessage message) {
    debugPrint('[FcmFlutterService] Notification tapped: ${message.data}');
    final tapped = Map<String, dynamic>.from(message.data);
    tapped['__event'] = 'tap';
    NotificationEvents.notify(RemoteMessage(data: tapped));
  }
}
