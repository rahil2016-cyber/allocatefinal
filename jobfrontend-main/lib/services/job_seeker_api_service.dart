...
class JobSeekerApiService {
  ...
  Future<Map<String, dynamic>> createResumePdfOrder({
    ...
    final r = await http.post(
      ...
      body: jsonEncode({
        ...
        if (resumeTemplateKey != null && resumeTemplateKey.isNotEmpty)
          'resume_template_key': resumeTemplateKey,
      }),
    );
    final json = _decode(r);
    _ensureSuccess(json, r.statusCode);
  ...
  /// `POST /job-seeker/payments/create-order` — create a Cashfree order.
  Future<Map<String, dynamic>> createCashfreeOrder(String packageKey) async {
    final r = await http.post(
  ...
  @Deprecated('Use createCashfreeOrder')
  Future<Map<String, dynamic>> createPhonePeOrder(String packageKey) =>
      createCashfreeOrder(packageKey);

  /// `POST /job-seeker/payments/confirm-status` — confirm Cashfree payment via Get Order.
  Future<Map<String, dynamic>> confirmCashfreePayment({
  ...
  @Deprecated('Use confirmCashfreePayment')
  Future<Map<String, dynamic>> confirmPhonePePayment({
    required String merchantOrderId,
  }) =>
      confirmCashfreePayment(merchantOrderId: merchantOrderId);
