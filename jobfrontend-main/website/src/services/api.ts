...
export const api = {
  ...
  reportJob(jobId: string, payload: { reason: string; description?: string }) {
    return request<Record<string, unknown>>(`/job-seeker/jobs/${jobId}/report`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  ...
  /** Creates a Cashfree PDF order (complete payment in the mobile app; web has no Cashfree SDK). */
  resumePdfPurchase(payload: { resume_template_id: number; resume_template_title: string }) {
    return request<Record<string, unknown>>("/job-seeker/resume/pdf-create-order", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  confirmCashfreePayment(payload: { merchant_order_id: string }) {
    return request<Record<string, unknown>>("/job-seeker/payments/confirm-status", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  ...
  adminJobReports(params?: { status?: string; page?: number; per_page?: number }) {
    const query = new URLSearchParams();
    if (params?.status) query.set("status", params.status);
    if (params?.page) query.set("page", String(params.page));
    if (params?.per_page) query.set("per_page", String(params.per_page));
    return request<Record<string, unknown>>(`/admin/job-reports${query.size ? `?${query.toString()}` : ""}`);
  },
  adminUpdateJobReport(reportId: number | string, payload: { status?: string; admin_note?: string }) {
    return request<Record<string, unknown>>(`/admin/job-reports/${reportId}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
  },
